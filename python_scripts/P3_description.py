...
def describe(cell_masks, nucleus_masks, t, thread_count, g, r , fr , ref_table):
    '''
    ...
    R=tif.imread(r)
    FR=tif.imread(fr)
    if np.max(G)>1: # the image isn't empty 
...
if __name__ == '__main__':
    ...
    ref_table=snakemake.input[5]

    thread_count=snakemake.threads

    t=int(g.split("ch2t")[1].split('.tiff')[0])

    file_name=cell_masks.split('/results/')[1]

    file_name=file_name.split('/')[0]

    exp=file_name.split('_')[0]
    pos=file_name.split('_')[1]
    results= describe(cell_masks, nucleus_masks, t, thread_count, g, r , fr , ref_table)
